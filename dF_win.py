    ...
    for i in range(raw_imaging.shape[1]):    
        ...
        for j,ft in enumerate(raw_filtered):
            ...
            if j-int(win_samples/2) < 0:
                f0 = np.percentile(raw_filtered[0:j+int(win_samples/2)],5) 
            elif j+int(win_samples/2) > raw_filtered.size:
                f0 = np.percentile(raw_filtered[j-int(win_samples/2):raw_filtered.size-1],5)
            else:
                f0 = np.percentile(raw_filtered[j-int(win_samples/2):j+int(win_samples/2)],5)
